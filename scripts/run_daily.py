...
def format_digest_text(digest):
    ...
    lines.append("")
    lines.append("─" * 60)
    lines.append("  ⚖️  LEGAL DISCLAIMER — READ BEFORE ACTING")
    lines.append("─" * 60)
    lines.append("  FOR INFORMATIONAL PURPOSES ONLY. NOT FINANCIAL ADVICE.")
    lines.append("  Nothing in this alert constitutes a recommendation to")
    lines.append("  buy, sell, or hold any security. Past price movements")
    lines.append("  following Trump statements are historical observations")
    lines.append("  only and do not predict future results.")
    lines.append("")
    lines.append("  OGE filings lag actual trades by weeks — holdings data")
    lines.append("  may be incomplete. Not all statements are captured;")
    lines.append("  video-only events and private calls are not monitored.")
    lines.append("")
    lines.append("  YOU ASSUME FULL RESPONSIBILITY for any decision made")
    lines.append("  based on this output. Always verify independently:")
    lines.append("  SEC.gov · OGE (efts.usoge.gov) · Reuters · AP News")
    lines.append("=" * 60)
